...
public class Utils {
    ...
    public static String buildMessage(List<? extends Crawlable> list) {
        ...
        s.append("<link rel='stylesheet' href='https://cdn.jsdelivr.net/npm/layui@2.9.13/dist/css/layui.css'>");
        s.append("<style>");
        s.append("body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; background-color: #f2f2f2; margin: 0; padding: 12px; color: #333; }");
        s.append(".zdm-table { width: 100%; max-width: 1100px; margin: 0 auto; border-radius: 8px; overflow: hidden; }");
        s.append(".zdm-table th { font-weight: 600; font-size: 14px; }");
        s.append(".zdm-table td { font-size: 14px; }");
        s.append(".zdm-table img { display: block; width: 110px; height: 110px; object-fit: cover; border-radius: 6px; border: 1px solid #eee; }");
        s.append(".zdm-table a { color: #1e9fff; text-decoration: none; font-weight: 600; line-height: 1.4; display: block; }");
        s.append(".zdm-table a:hover { color: #16b777; }");
        s.append(".zdm-table td:nth-child(3) { color: #ff5722; font-weight: 700; }");
        s.append(".zdm-table td:nth-child(4) { color: #999; font-size: 13px; }");
        s.append(".zdm-table td:nth-child(5) { color: #666; font-size: 13px; }");
        s.append("@media (max-width: 600px) {");
        s.append("  thead { display: none; }");
        s.append("  tr { display: block; margin-bottom: 12px; background: #fff; border-radius: 8px; padding: 12px; position: relative; box-shadow: 0 2px 8px rgba(0,0,0,0.06); border: 1px solid #eee; }");
        ...
        s.append("  td:nth-child(3) { color: #ff5722; font-size: 16px; margin-bottom: 6px; }");
        s.append("  td:nth-child(4) { display: inline-block; width: auto; margin-right: 10px; font-size: 12px; color: #999; }");
        s.append("  td:nth-child(5) { display: inline-block; width: auto; margin-left: 0; font-size: 12px; background: #f2f2f2; padding: 2px 6px; border-radius: 4px; color: #666; }");
        s.append("}");
        s.append("</style></head><body>");
        s.append("<div id='app' style='display:none'>");
        s.append("<table class='layui-table zdm-table'>");
